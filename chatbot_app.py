...
#         return models
#     except Exception:
#         return ["gemma3:1b","gemma3:270m", "qwen3:1.7b"]
...
#     return ensemble_retriever, docs


# ensemble_retriever, all_docs = load_system()
...
#     router_llm = ChatOllama(model=selected_model, temperature=0)

#     start = time.perf_counter()

#     decision = llm_route_query(router_llm, user_query, st.session_state.history)
...
#     if decision.use_rag:
#         with st.spinner("🔍 Đang tìm máy phù hợp..."):
...
#                     # --- 1. HIỂN THỊ MÔ TẢ (Chuyên nghiệp & Tối ưu) ---
#                     if desc_text:
...
#                         else:
#                             desc_list = [d.strip() + '.' for d in raw_desc.split('.') if len(d.strip()) > 1]
...
#                     # --- 2. HIỂN THỊ ĐẶC ĐIỂM (Đồng bộ format với Mô tả) ---
#                     if features:
...
#                         img_html = "".join(
#                             [
#                                 f'<img src="{img}" style="height:160px;margin-right:10px;border-radius:8px;border:1px solid #ddd;object-fit:contain;max-width:200px;">'
#                                 for img in images
#                             ]
#                         )
...
#         # ----------------------------
#         # SPEC QUERY (Cải tiến hiển thị Full Thông số)
#         # ----------------------------
...
#             # Câu trả lời ngữ cảnh từ LLM
#             answer = build_specs_answer(user_query, best_doc)

#             if best_doc:
#                 content = best_doc.page_content
                
#                 # Trích xuất dữ liệu gốc để không bị LLM tóm tắt mất
#                 # Bạn có thể đổi chữ "Specifications" thành tên trường cấu hình thực tế trong CSV của bạn (ví dụ "Thông số")
#                 raw_specs = get_field(content, "Specifications") 
                
#                 if raw_specs:
#                     clean_specs = raw_specs.replace(r'\n', '\n')
#                     # Nối thẳng thông số kỹ thuật gốc vào dưới câu trả lời của LLM
...
#     st.caption(f"⏱ Thời gian trả lời: **{latency}s** | Model: `{selected_model}`")
